...
int main(int argc, char *argv[]) {
	...
	mysql_client* mysql = new mysql_client("test","localhost","test","tset");
		...
		*readfds = *allfds;
		...
		if ( noready < 0 ) {	///// ERROR
			...
			if ( FD_ISSET(STDIN_FILENO, readfds) ) {
				...
				// bridge stdin -> mysql & subserver			
				if ( packet.type == PKT_DATA  && packet.datalen() > 1){
					mysql->getRowQuery("INSERT INTO `test`.`test_table` (`id` ,`value`) "
								"VALUES (NULL , '5');");
					printf("processing sql\n");
				}
				me.sendpacket(packet);
		#endif
			}
