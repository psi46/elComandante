...
valuedconst_t::valuedconst_t(string Unit, Double_t Value, flags_t Flags) : value_t(Unit, Flags) {
	value = Value;
}

int valuedconst_t::scan_value(char*& ptr) {
	char* eptr=ptr;
	skip_ws(eptr);
...
string valuedconst_t::String() {
	ostringstream os(ostringstream::out);
	os << *this;
	return os.str();
}
Double_t valuedconst_t::Double() {
	return value;
}
Integer_t valuedconst_t::Integer() {
	return (Integer_t)value;
}
//
// GLOBAL SCOPE STREAM OUT OPERATOR
//
ostream& operator<<(ostream& os, valuedconst_t& val) {
