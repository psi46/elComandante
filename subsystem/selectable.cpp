...
selectable::~selectable() {
		sel[thissel] = sel[--nosel];	// copy pointer down
		sel[nosel] = NULL;		// unset old pointer
		if (sel[thissel] != NULL)
			sel[thissel]->thissel = thissel;	// give its new array position to the class
	}
...
int selectable::run(long sec, long usec) {
		...
		for (int i=0; i<nosel; i++) { // for each selectable
			...
			fd = sel[i]->getfd();
			if (fd < 0) {
				if (DEBUG) eprintf("%s:%d WARNING: invalid file descriptor\n", __FILE__, __LINE__);
				continue;
			}
