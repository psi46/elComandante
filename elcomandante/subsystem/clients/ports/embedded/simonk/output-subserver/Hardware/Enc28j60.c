...
//MAC and MII registers need a dummy byte while reading them (REGISTER_NEEDDUMMY)
#define REGISTER_MASK			0x1F
#define REGISTER_NEEDDUMMY		0x80

//BankX: (means: available in each bank)
...
//Useful macros
#define SELECT()		ENC28J60_PORT &= ~(1<<ENC28J60_CS)
#define DESELECT()		ENC28J60_PORT |= (1<<ENC28J60_CS)
#define NOP()			asm volatile ("nop");

static
#ifdef ENC28j60_INLINE
...
uint8_t Enc28j60ReadReg(uint8_t nRegister)
{
	...
	SELECT();
	SpiWriteByte(OPCODE_RCR | (nRegister & REGISTER_MASK));
...
{
	SELECT();
	SpiWriteByte(OPCODE_WCR | (nRegister & REGISTER_MASK));
	SpiWriteByte(nData);
	DESELECT();
}
...
{
	SELECT();
	SpiWriteByte(OPCODE_BFS | (nRegister & REGISTER_MASK));
	SpiWriteByte(nMask);
	DESELECT();
}
...
{
	SELECT();
	SpiWriteByte(OPCODE_BFC | (nRegister & REGISTER_MASK));
	SpiWriteByte(nMask);
	DESELECT();
}
...
{
	//No read/modify/write here! Although it could be faster
	//It is important to use built-in Bit-Set and Bit-Clear functions
	//to make this access atomic
	Enc28j60ClearMaskReg(BANKX_ECON1, (3<<BANKX_ECON1_BSEL0));
	Enc28j60SetMaskReg(BANKX_ECON1, (nBank << BANKX_ECON1_BSEL0));
}

//PHY OPERATIONS CHANGE CURRENT BANK!
static
#ifdef ENC28j60_INLINE
inline __attribute__ ((always_inline))
#endif
...
void Enc28j60Init()
{
	...
	//we clear the FullDuplex Bit ourself
...
uint16_t Enc28j60Receive(uint8_t* pBuffer)
{
	...
	Enc28j60WriteReg(BANK0_ERDPTL , (nNextPacket >> 0));
	Enc28j60WriteReg(BANK0_ERDPTH , (nNextPacket >> 8));
	...
	{
		//Overflow occured while subtracting 1? Program RXEND to the NextPacketPointer
						...
						uint16_t nBytes)
{
	...
	//This workaround will wait for TXRTS to be cleared within a maximum of 500ms
	uint8_t i = 50;
	while(i--)
	{
	...
	Enc28j60WriteReg(BANK0_ETXNDL, (uint8_t) ((ENC28J60_TXSTART + nBytes) >> 0));
	Enc28j60WriteReg(BANK0_ETXNDH, (uint8_t) ((ENC28J60_TXSTART + nBytes) >> 8));

	SELECT();
	SpiWriteByte(OPCODE_WBM);
	SpiWriteByte(0);
	SpiWriteChunk(pBuffer, nBytes);
	DESELECT();
