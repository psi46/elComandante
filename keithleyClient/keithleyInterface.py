...
class keithleyInterface:
    ...
    def setCurrentMeasurmentRange(self, measurement_range):
        if not self.validCurrent(measurement_range):
            raise Exception('setting CurrentMeasurmentRange: not valid current: %s' % measurement_range)
        return self.write(':SENS:CURR:RANG %s' % measurement_range)
    ...
    def setSweepSpacingType(self, spacing):
        if spacing not in ['LIN', 'LOG']:
            raise Exception('Sweep Spacing Type not valid %s' % spacing)
        return self.write(':SOUR:SWE:SPAC %s' % spacing)
    ...
    @staticmethod
    def validVoltage(value):  # TODO Write function which 'knows' if the voltage is possible
        return True

    @staticmethod
    def validCurrent(current):  # TODO
        return True
